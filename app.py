...
def precipitation():
    ...
    results = session.query(Measurement.date, func.avg(Measurement.prcp)).\
        filter(Measurement.date.between(
            dates, dt.date(2017, 8, 23))).group_by('date')
    session.close()
...
def start(start_date):
    ...
    if len(start_date) == 10:
        ...
        for r in range_calc:
            range_list.append(
                f'date: {r.Date} Min temperature:{r.Min} Avg temperature:{round(r.Avg,2)}, Max temperature: {r.Max}')
    else:
        range_list.append(
            f'Incorrect date {start_date} format, should be YYYY-MM-DD')
...
def range(start_date, end_date):
    ...
    #     for date in range_calc:
        ...
        range_list.append(
            f'Incorrect date {start_date}/{end_date} format, should be YYYY-MM-DD/YYYY-MM-DD')
